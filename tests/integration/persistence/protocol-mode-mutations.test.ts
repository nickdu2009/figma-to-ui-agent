 ...
 *
 * 验证：
 * 1. compat 模式：正常执行 preview-commit / publish / rollback / runtime-actions；
 * 2. cutover 模式：所有写入端点返回 423 protocol_mode_cutover_fence_active；
 * 3. readonly_recovery 模式：所有写入端点返回 423 protocol_mode_readonly_recovery_active，只读端点正常工作。
 */
...
import { createPreviewSelectionRoutes } from "../../../server/routes/preview-selection.ts";
...
describe("S13 协议模式 Mutation 门禁集成测试", () => {
  ...
  it("cutover 模式下发布端点返回 423 protocol_mode_cutover_fence_active", async () => {
    const app = createReleaseRoutes({
      authService: mockAuthService,
      appRepository: mockAppRepo,
      releaseRepository: releaseRepo,
      releaseService,
      protocolMode: "cutover",
    });

    const res = await app.request(`/apps/${seed.appId}/releases/publish`, {
      method: "POST",
    ...
    expect(body.error?.code).toBe("protocol_mode_cutover_fence_active");
  });

  it("cutover 模式下 Preview Commit 返回 423", async () => {
    const app = createPreviewSelectionRoutes({
      authService: mockAuthService,
      appRepository: mockAppRepo,
      releaseRepository: releaseRepo,
      previewSelectionRepository: selectionRepo,
      protocolMode: "cutover",
    });

    const res = await app.request(`/apps/${seed.appId}/preview-commit`, {
      method: "POST",
      headers: AUTH_HEADERS,
      body: JSON.stringify({
        generationId: "gen-1",
        candidateDigest: "cd-1",
        uiBundleDigest: "ubd-1",
        reportDigest: "rd-1",
        result: { status: "ready", specStatus: "ready", revision: 1 },
      }),
    });
