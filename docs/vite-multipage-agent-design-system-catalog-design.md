...
- 状态：已确认方案；第三轮设计审核修订完成，独立增量，尚未制定实施计划
...
- 完整 ApplicationCandidate 的原子生成与验证，以及 AppUiBundle + Business Schema/Policy/迁移聚合的草稿、预览、发布和回滚。
...
flowchart LR
  ...
  Catalog["CatalogContract\n组件 + Action + Prompt"] --> Generate
  Generate --> Candidate["ApplicationCandidate\nAppUiBundle + Business Schema/Policy"]
  Candidate --> StaticValidator["服务端静态 Validator\nUI G0 + Business Schema/Policy/Migration"]
  StaticValidator --> ValidationRunner["服务端控制的 Playwright Validation Runner\n隔离渲染 + 权威 G1/G2"]
  ValidationRunner --> Staging["用户浏览器隐藏 Staging iframe\nruntime apply + digest smoke"]
  Staging --> Commit["幂等 Preview Commit API\n创建不可变 DraftVersion"]
  Commit --> Host["宿主 BundleApplyController\n确认持久化后交换可见 iframe"]
...
1. CatalogContract 是组件、children/compound 关系、Event、Action 参数与模型提示的唯一权威来源。
...
7. 服务端静态 Validator 只负责无需浏览器即可确定的 B0/G0；计算样式、对比度和交互状态由服务端控制的 Playwright Validation Runner 在真实浏览器中检查。用户浏览器 staging 只验证当前客户端能否加载/apply，不是发布质量事实 owner。
...
type ApplicationCandidate = {
  ...
  businessSchema: AppDataSchemaCandidate;
  dataAccessPolicy: DataAccessPolicyCandidate;
  migrationPlan?: DataMigrationPlanCandidate;
  reverseMigrationPlan?: DataMigrationPlanCandidate;
};
...
`AppUiBundle` 是应用 UI 的唯一事实。`ApplicationCandidate` 只是 GenerationRun 拥有的协调包，不成为新的持久业务事实：提交 DraftVersion 时分别保存不可变 AppUiBundle，并按既有发布模型保存/引用 Business Schema、DataAccessPolicy 和迁移计划。编译后的 CSS、Token 展平结果、预加载资源表和渲染快照都是可重建派生物，不是第二事实源。`AssetManifest` 只保存不可变资源描述与内容哈希，二进制 Blob 的事实 owner 是 Asset Pipeline。
...
既有发布领域继续拥有 `DraftVersion`、`PublishedVersion` 和 `ReleasePointer`。这些实体由原先保存/引用 Spec，扩展为保存/引用不可变 `AppUiBundle`；业务 Schema 与数据权限策略仍由既有发布聚合管理，不塞入 UI Bundle。
...
- DraftVersion 必须使用服务端已验证的 ApplicationCandidate 创建；其中 UI 部分取 candidate.uiBundle，业务 Schema/Policy/迁移按既有发布聚合分别保存，不能序列化 iframe 当前 StateStore。
...
- Action 名、参数 Zod Schema、结果 Schema、错误码和权限类别。
- 稳定样式选择器与 Token 映射。
- Catalog 版本和兼容性信息。
...
1. 服务端 `modelCatalog`、Spec 组件/Action prompt fragment 和 Bundle Prompt。
...
React Renderer 函数本身不是纯数据，不能由 Zod 元数据生成，也不能进入服务端 CatalogContract。browser-only `RendererBindings` 显式提供实现；`defineRegistry` 只能接收该绑定。TypeScript `satisfies` 与运行时门禁必须同时断言 bindings、Catalog components 和最终 registry 的键完全相等，缺失、多余或版本不匹配均拒绝构建/启动。这样保持 CatalogContract 是能力契约唯一事实，同时承认 Renderer 实现有独立代码 owner。
...
业务 Actions 按能力分为 6 组，共 11 个精确动作。为区分设计资源，原 `uploadAsset` 明确命名为 `uploadAttachment`；设计资料上传由宿主聊天/资源入口处理，不是生成应用 Action。
...
现有 `setState`、`pushState`、`removeState` 继续用于纯客户端交互，不计入上述业务 Action 清单。
...
| --- | --- | --- | --- |
| `published` | 读取当前 PublishedVersion 对应业务 Schema | 按固定角色上限、已发布策略、记录范围、字段权限与 expectedRevision 执行 | 当前已发布 DataAccessPolicyVersion |
...
外部应用修改入口仍只有 `generate_spec`，但其内部编辑目标从 NextAppSpec 升级为完整 ApplicationCandidate。聊天 Agent 不携带完整 Candidate；服务端根据已认证 appId、opaque baseRef 与 baseDigest 解析当前不可变 Draft/Published UI Bundle、Business Schema、DataAccessPolicy 与适用迁移上下文，并把它们作为生成器私有上下文：
...
`get_current_spec` 在 Catalog 1.x 中保留为兼容的只读事实工具，用于普通问答和结构摘要，并把当前预览对应的 opaque `baseRef/baseDigest` 一并返回；编辑不再信任聊天模型回传的 `currentSpec/currentBundle/businessSchema`。生成服务按 baseRef 加载同应用的不可变版本聚合，并核对 draft revision（如适用）与 digest，避免多草稿歧义，也避免把完整 Candidate 暴露给聊天模型或由浏览器参数替换。

内部 RFC 6902 operation 的根对象是 ApplicationCandidate，允许路径为 `/uiBundle/spec/**`、`/uiBundle/designSystem/**`、`/uiBundle/assets/**`、`/businessSchema/**`、`/dataAccessPolicy/**`、`/migrationPlan/**` 与 `/reverseMigrationPlan/**`。创建基线必须包含完整默认 AppUiBundle、空 Business Schema、不会扩权的默认 Policy，迁移计划默认缺省。UI 路径由 CatalogContract/Bundle Gate 校验，业务路径复用既有 Business Schema、Policy 收紧和迁移 Schema 校验，不能用一个宽泛 JSON Schema 代替。结构化 operation 工具继续每批有界提交，但 CUSTOM 事件升级为版本化的 `app.candidate.patch.start/delta/finish/error`。部署切换时中止所有未完成旧 run，再同时切换服务器和浏览器协议；旧 `spec.patch.*` 不与新协议双栈运行，也不恢复或重放。
...
生成器 LLM
  ...
  -> 服务端 Gate：UI G0 + Business Schema/Policy/迁移校验
...
type PreviewCommitResponse =
  ...
  | { status: "run_failed"; candidateDigest: string };

POST /apps/:appId/generations/:generationId/preview-result
```

可信 appId 只来自 URL path。服务端要求同一应用、`GenerationRun.awaiting_preview`、精确 candidateDigest 与服务端已保存的 reportDigest，并以 `(generationId, candidateDigest)` 作为幂等键；DraftVersion 对 sourceGenerationId 唯一。相同请求重复到达返回第一次的稳定结果，不重复建草稿；错应用、错 digest/reportDigest、迟到、已标记 incomplete 或不同第二结果一律拒绝。`staged` 在单个 MySQL 事务内使用 GenerationRun 中的权威 ValidationReport 创建 DraftVersion 并把 run 转为 succeeded；浏览器不能提交或覆盖 report、issues、publishBlocked。`failed` 把 run 转为 failed，只保存有界诊断。
...
7. Draft 已持久化但本地 iframe 交换失败时，DraftVersion 与 run 保持 succeeded，当前标签页显示“草稿已保存，预览需刷新”并继续显示旧预览；刷新后从该 DraftVersion 重建，不反向删除成功草稿。
8. 成功交换后使用尊重 `prefers-reduced-motion` 的 160ms opacity 淡入；减少动态偏好下不播放动画。
9. 既有 GenerationRun 的 `candidateSpec/candidateBusinessSchema` 扩展为 `candidateBundle/candidateBusinessSchema/candidateDataAccessPolicy/candidateMigrationPlan`；它们共同计算 candidateDigest。浏览器只提交 StagingResult，服务端 Preview Commit Response 才代表整个 DraftVersion 聚合持久化完成。
...
| Gate | 行为 | 例子 |
| --- | --- | --- |
| B0 业务契约 | 拒绝整个 ApplicationCandidate；不进入 staging | Business Schema、Policy 收紧性、迁移/反向迁移或资源上限失败 |
...
4. 结果：任一路由无法加载、矩阵未完整执行、审计超时或报告与 candidateDigest/Profile 版本不匹配，均产生 `publishBlocked: true`，不得按通过处理。
...
type CandidateValidationResult =
  ...
  | {
      status: "rejected";
      code: "business_validation_failed" | "bundle_validation_failed" | "staging_apply_failed";
      candidateDigest?: `sha256:${string}`;
      issues: ValidationIssue[];
      truncated: boolean;
    };
...
- `400` 表示请求结构错误，`403` 表示无权操作，`409` 表示 Revision 冲突，`413` 表示字节超限，`422` 表示 Candidate 的 B0/G0/G1 语义失败。发布请求命中已保存的 G1 `publishBlocked` 时由 Release API 返回独立的 `publish_validation_failed`，不伪装成 Preview Commit 拒绝。
...
1. 增加 nullable `candidateBundle/bundle/catalogVersion/validationReport/publishBlocked/candidateDigest/candidateDataAccessPolicy/candidateMigrationPlan`，保留既有 candidateBusinessSchema/businessSchema，并保证 DraftVersion 的 `sourceGenerationId` 唯一，使 PreviewResult 幂等重放不能创建第二个草稿。
2. 读路径优先读取 Bundle；旧行缺少 Bundle 时通过 legacy adapter 动态包装默认 DesignSystem 与空 AssetManifest，不修改原始 spec。
3. 后台按确定性规则回填旧行，逐行使用当前 Catalog 1.x 校验并记录有界结果；任一失败停止切换，不删除旧列。
4. 完成回填校验后，新 GenerationRun/Draft/Published 写入以 Bundle 为事实，同时把 `bundle.spec` 写入旧 spec 列作为只读兼容投影，使前一版本服务仍可回滚读取。
5. 兼容投影不得独立更新；Repository 接口只接受 Bundle，并在同一事务派生 spec。检测到两者不一致时 fail closed。
6. 至少完成一次服务回滚演练和全部已发布版本读取验证后，才可在未来独立迁移中考虑删除旧列；本文不授权删除。

数据库回滚只回滚应用服务与读写路径，不反向删除已经安全增加的 nullable 列或 Bundle 数据。这样不会把新 Bundle 降级丢失为只有 Spec 的版本。

协议切换时先部署能够读取旧列、但尚不发出 `app.candidate.patch.*` 的兼容服务，再完成回填与 hostname/CSRF 探针，最后中止旧未完成 run 并原子切换服务器事件协议和浏览器客户端。回滚到旧服务时只读取兼容 spec 投影；已创建的新 Bundle/Draft 不删除，重新升级后仍可恢复读取。
...
| Playwright Validation Runner | Platform Validation Runtime | Candidate + ValidationProfile → 权威 ValidationReport/reportDigest | 发布决定、业务数据 |
...
每个事实只有一个 owner：Bundle 内容由不可变版本拥有；Catalog 能力定义由 CatalogContract 拥有，RendererBindings 只是与其键闭合的代码实现；设计 Blob 由 Design Asset Pipeline 拥有；Asset Capability 是可撤销派生凭据，不是资源事实；BusinessAttachment 元数据与业务记录由既有业务数据模块拥有，BlobStore 只拥有不可变文件正文；当前发布指针由既有发布模块拥有。DraftVersion 需扩展保存 AppUiBundle、ValidationReport、publishBlocked 与 catalogVersion；PublishedVersion 保存发布时相同的 Bundle 与 catalogVersion。
...
7. **业务 Actions 与附件**：11 个精确 Action、iframe Adapter、published/draft 执行矩阵，接入既有 Hono API、权限和乐观并发。
8. **生成、提交、发布与迁移**：ApplicationCandidate Patch 协议、GenerationRun candidateBundle/Schema/Policy/Migration、幂等 Preview Commit、Draft/Published 聚合与 ValidationReport、完整 Bundle 原子交换/淡入、Catalog 1.x 兼容和发布/回滚 Gate。
...
- AC4b：Application Prompt 不包含原生 NextAppSpec 的旧 sample-data/root-output 指令；Prompt 契约测试断言 Patch 根为 ApplicationCandidate、UI 持久 state 只允许 `/ui`，Business Schema/Policy/迁移只写各自受控根路径。
- AC4c：candidateDigest 由 canonical ApplicationCandidate 全量计算；UI Bundle、Business Schema、Policy 或任一迁移计划变化都会改变 digest，PreviewResult 不能只确认 UI 子集。
...
- AC13c：G1 运行前后 ApplicationCandidate digest 完全一致；任何自动修复产生新 generationId/digest 并重新通过 Business Schema/Policy/Migration Gate 与 UI G0/G1/G2。
- AC13d：服务端 Validation Runner 生成的 ValidationReport 覆盖全部静态路由、每个动态路由至少一个 staticParams、桌面/移动视口和声明的关键状态；任一 case 缺失、Runner 失败或报告错 candidateDigest/profile 时不进入 Preview Commit，完整执行但发现 G1 问题时 publishBlocked。
...
- AC15：提交成功后执行 160ms opacity 淡入；启用 `prefers-reduced-motion` 时不播放该动画。
