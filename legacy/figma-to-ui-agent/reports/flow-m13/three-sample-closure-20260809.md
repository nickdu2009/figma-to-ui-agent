# Flow-M13 three-sample restricted-live closure
...
- Cake 样本在单样本 pre-fix run 中只能证明部分 navigate；Canvas 上下文回填后，三样本 run 中 `trustedNavigate` 提升到 36，说明 destination 回填和后续目标页补抓链路有效。
...
1. 将三样本收口结果沉淀为 Worktrail validation candidate，等待 review/promote。
2. 再进入 `ui_node_not_clickable` / `prototype_target_page_missing` 泛化修复，目标是降低 Cake 样本剩余 unsupported 和 missingEvidence。
3. 修完后用同一三样本组合复跑，确认 `trustedNavigate`、`trustedStateChange` 和 submit-like confirmation 不回退。
